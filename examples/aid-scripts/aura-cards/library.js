...
// normally — no context hijack, no story pause. When the AI module isn't
// mounted, this layer is fully inert and the original Auto-Cards behavior
// (which uses an in-game prompt-injection turn) is preserved unchanged.
//
// The integration touches only three points in the original script:
//   1) This top-level IIFE provides the Frontier client.
//   2) `promptGeneration()` checks AuraFrontier.available() before doing
//      its context hijack, and if so queues a chat request instead.
//   3) The output hook polls AuraFrontier for completed responses and
//      applies them via the existing `constructCard` helper so all
//      formatting/limit logic stays consistent.
// All AuraFrontier state lives in `state.aura` to avoid touching
// Auto-Cards' validated state schema.
// =============================================================================

var AuraFrontier = (function () {
    "use strict";

    var OUT_CARD       = "frontier:out";
    var IN_CARD        = "frontier:in:ai";
    var HEARTBEAT_CARD = "frontier:heartbeat";
    ...
    var MAX_ACK_RETRIES = 6;

    function ensureState() {
        state.aura = state.aura || {
            seq: 0, outSeq: 0,
            pending: {},        // requestId -> { title, queuedAt, request, ackAttempts }
            recent: {},         // title -> { requestId, status, appliedAt, error }
            events: [],
            _acks: []
        };
        if (!state.aura.pending) state.aura.pending = {};
        if (!state.aura.recent)  state.aura.recent  = {};
        if (!state.aura.events)  state.aura.events  = [];
        if (!state.aura._acks)   state.aura._acks   = [];
        return state.aura;
    }

    // --- AID story-card helpers (defensive, never throw) -------------------
    function cards() { return Array.isArray(storyCards) ? storyCards : []; }
    ...
    function readJson(title) {
        var f = findCard(title);
        if (!f.card) return null;
        var raw = f.card.value || f.card.entry || f.card.description || "";
        try { return JSON.parse(raw || "{}"); } catch (e) { return null; }
    }
    function writeCard(title, value, type) {
        var f = findCard(title);
        var t = type || "Frontier";
        if (f.card && f.index >= 0 && typeof updateStoryCard === "function") {
            updateStoryCard(f.index, f.card.keys || f.card.key || title, value, f.card.type || t);
            return true;
        }
        if (typeof addStoryCard === "function") {
            addStoryCard(title, value, t);
            return true;
        }
        return false;
    }
    ...
    function logEvent(event, detail) {
        var s = ensureState();
        s.events.push({ at: nowMs(), event: event, detail: String(detail || "") });
        while (s.events.length > 40) s.events.shift();
    }

    // --- heartbeat probe ---------------------------------------------------
    // Single source of truth for "is the Frontier AI module mounted right
    // now?". Cheap to call (one card read + small JSON parse) so it's safe
    // to invoke on every hook.
    function available() {
        var hb = readJson(HEARTBEAT_CARD);
        if (!hb || !hb.frontier) return false;
        if (hb.frontier.protocol !== 1 || hb.frontier.profile !== "full") return false;
    ...
    function pendingArray() {
        ...
        for (var id in s.pending) {
            ...
            var p = s.pending[id];
            if (p && p.request) out.push(p.request);
        }
        return out;
    }
    function flushOut() {
        ...
        var payload = {
            v: 1,
            requests: pendingArray(),
            acks: s._acks || [],
            debugSeq: ++s.outSeq,
            debugWrittenAt: nowMs()
        };
        s._acks = [];
        writeCard(OUT_CARD, JSON.stringify(payload), "Frontier");
    }

    function queueChat(title, prompt) {
        var s = ensureState();
        var id = liveKey() + "-aura-" + (++s.seq);
        var args = {
            provider: "openrouter",
            messages: [
                { role: "system", content: "You are a story-card archivist for an AI-Dungeon-style narrative game. Write only the requested informational entry. Do not narrate, role-play, or continue the story. Use complete sentences with correct punctuation." },
                { role: "user",   content: String(prompt || "").slice(0, 7800) }
            ],
            maxTokens: MAX_TOKENS,
            temperature: TEMPERATURE,
            timeoutMs: TIMEOUT_MS
        };
        if (DEFAULT_MODEL) args.model = DEFAULT_MODEL;
        s.pending[id] = {
            title: title,
            queuedAt: nowMs(),
            request: { id: id, module: "ai", op: "chat", args: args, ts: nowMs() },
            ackAttempts: 0
        };
        logEvent("queued", id + " for '" + title + "'");
        return id;
    }
    ...
    function pollResponse(requestId) {
        ...
        var card = readJson(IN_CARD);
        if (!card || !card.responses) return null;
        var r = card.responses[requestId];
        if (!r) return null;
        if (r.status === "ok" || r.status === "err" || r.status === "timeout") return r;
        return null;
    }
    function ack(requestId) {
        var s = ensureState();
        var entry = s.pending[requestId];
        var attempts = entry ? Number(entry.ackAttempts || 0) : Number((s.recent[requestId] && s.recent[requestId].ackAttempts) || 0);
        if (attempts >= MAX_ACK_RETRIES) return false;
        s._acks = s._acks || [];
        s._acks.push(requestId);
        if (entry) entry.ackAttempts = attempts + 1;
        return true;
    }
    function complete(requestId, status, error) {
        var s = ensureState();
        var entry = s.pending[requestId];
        ...
        if (title) {
            s.recent[title] = {
                ...
                appliedAt: nowMs(), error: error || null
            };
        }
        logEvent("complete", requestId + " " + status + (error ? " (" + (error.code || "") + ")" : ""));
    }
    function abort(requestId, reason) {
        var s = ensureState();
        if (s.pending[requestId]) {
            var title = s.pending[requestId].title;
            delete s.pending[requestId];
            logEvent("abort", requestId + " " + (reason || ""));
            if (title) {
                s.recent[title] = { requestId: requestId, status: "abort", appliedAt: nowMs(), error: { code: "abort", message: reason || "" } };
            }
        }
    }
    ...
    function status(extra) {
        ...
        var hb = readJson(HEARTBEAT_CARD);
        var lines = [];
        lines.push("Aura Cards — Frontier AI status");
        lines.push("AI module: " + (available()
            ? "ready (Frontier-driven generation active)"
            : "unavailable (falling back to standard Auto-Cards behavior)"));
        if (hb && hb.frontier) {
        ...
        for (var id in s.pending) {
            if (!Object.prototype.hasOwnProperty.call(s.pending, id)) continue;
            pending.push("  - " + s.pending[id].title + " (" + id + ")");
        }
        lines.push("Pending: " + (pending.length === 0 ? "none" : ""));
        if (pending.length) lines = lines.concat(pending);

        var recentTitles = Object.keys(s.recent || {});
        if (recentTitles.length) {
            lines.push("Recent (last 6):");
            recentTitles = recentTitles.slice(-6);
            for (var i = 0; i < recentTitles.length; i++) {
                var r = s.recent[recentTitles[i]];
                var tail = r.error ? (" — " + (r.error.code || "err")) : "";
                lines.push("  - " + recentTitles[i] + ": " + r.status + tail);
            }
        }
        lines.push("Events:");
        for (var j = Math.max(0, s.events.length - 8); j < s.events.length; j++) {
            var ev = s.events[j];
            lines.push("  " + ev.event + ": " + ev.detail);
        }
        if (extra) lines.push("\n" + String(extra));
        writeCard(STATUS_CARD, lines.join("\n"), "Aura");
    }
...
function AutoCards(inHook, inText, inStop) {
    ...
    */
    ...
    if (!HOOK) {
        ...
        case "context": {
            ...
            function promptGeneration() {
                ...
                // `isGenerating` false so the output hook treats this turn
                // as ordinary story progression. The completed entry is
                // applied later by the Aura output-side polling block.
                if (AuraFrontier.available()) {
                    ...
                    if (!stillPending) {
                        ...
                        // the Frontier AI module instead.
                        const partialEntry = formatEntry(wp.entry);
            ...
            break; }
        case "output": {
            // AURA: apply any completed Frontier-driven generations BEFORE
            // entering the original output flow. If the response is ready,
            // we synthesize a finished entry from the model's text and run
            // it through the same `constructCard` path the original code
            // path uses, so all formatting and limit logic stays identical.
            // If it's still in flight we just no-op and let the original
            // code treat this as a normal story turn.
            (function applyAuraIfReady() {
                ...
                if (response.status !== "ok") {
                    // Transient or terminal failure. Clear the request id so
                ...
                // Normalize the model's text the same way the original
                // capture path did: prettify dashes, strip markdown emphasis,
                // collapse whitespace, drop dialogue/say-style sentences,
                // dedupe against any partial entry already accumulated.
                const rawText = (response.data && typeof response.data.text === "string")
                    ? response.data.text : "";
                const normalized = prettifyEmDashes(rawText);
                const oldSentences = (splitBySentences(formatEntry(wp.entry))
                    .map(sentence => sentence.trim())
                    .filter(sentence => (2 < sentence.length))
                );
                const seenSentences = new Set();
                const entryAddition = splitBySentences(normalized
                    .replace(/[\*_~]/g, "")
                    .replace(/:+/g, "#")
                    .replace(/\s+/g, " ")
                    ...
                    if (/(?<=^|\s|—|\(|\[|{)sa(ys?|id)(?=\s|\.|\?|!|,|;|—|\)|\]|}|$)/i.test(sentence)) return false;
                    if (sentence.includes("\"")) return false;
                    if (oldSentences.some(s => 0.75 < similarityScore(s, sentence))) return false;
                    if ([...seenSentences].some(s => 0.75 < similarityScore(s, sentence))) return false;
                    seenSentences.add(sentence);
                    return true;
                }).join(" ").trim() + " ";

                if (entryAddition.trim() === "") {
                ...
                if (
                    /^{title:[\s\S]*?}$/.test(wp.entry.trim())
                    && (wp.entry.length < 111)
                ) {
                    wp.entry += "\n" + entryAddition;
                } else {
                    wp.entry += entryAddition;
                }
                if (wp.limit < wp.entry.length) {
                    // Same length-trim logic as the original capture path,
                    // but inlined so we don't have to refactor the original.
                    let exit = false;
                    let truncatedEntry = wp.entry.trimEnd();
                    ...
                    for (let i = sentences.length - 1; 0 <= i; i--) {
                        if (!sentences[i].includes("\n")) {
                            ...
                            if (truncatedEntry.length <= wp.limit) break;
                            continue;
                        }
                        ...
                        for (let j = lines.length - 1; 0 <= j; j--) {
                            ...
                            truncatedEntry = sentences.join("").trimEnd();
                            if (truncatedEntry.length <= wp.limit) { exit = true; break; }
                        }
                        if (exit) break;
                    }
                    if (truncatedEntry.length < 150) {
                        wp.entry = limitString(wp.entry, wp.limit);
                        const tail = wp.entry.replace(/\s*\S+$/, "");
                        if (150 <= tail.length) wp.entry = tail;
                    } else {
                        wp.entry = truncatedEntry;
                    }
                }
                wp.entry = wp.entry.trimEnd();

                // Final card construction. This is the exact same call the
                // original capture path makes, so bullet-mode formatting,
                // title headers, and the 2000-char card limit all behave
                // identically. The only difference is that the entry came
                // from a Frontier chat completion instead of the player's
                // hijacked turn.
                notify("\"" + wp.title + "\" was successfully added to your story cards!");
                constructCard(O.f({
                    type: wp.type,
                    title: wp.title,
                    keys: wp.keys,
                    entry: (function () {
                        if (!AC.config.bulletedListMode) return wp.entry;
                        const sentences = splitBySentences(
                            formatEntry(wp.entry.replace(/\s+/g, " ")).replace(/:+/g, "#")
                        ).map(sentence => {
                            sentence = (sentence
                                .replaceAll("#", ":")
                                .trim()
                        ...
                        const titleHeader = "{title: " + wp.title + "}";
                        if (sentences.every(sentence => (sentence.length < 12))) {
                            const sentencesJoined = sentences.join(" ").trim();
                            if (sentencesJoined === "") return titleHeader;
                            return limitString(titleHeader + "\n" + sentencesJoined, 2000);
                        }
                        for (let i = sentences.length - 1; 0 <= i; i--) {
                            const bulletedEntry = cleanSpaces(titleHeader + sentences.join(" ")).trimEnd();
                            if (bulletedEntry.length <= 2000) return bulletedEntry;
                            if (sentences.length === 1) break;
                            sentences.splice(i, 1);
                        }
                        return limitString(wp.entry, 2000);
                    })(),
                    description: wp.description,
                }), newCardIndex());

                // Reset the same fields the original capture path resets so
                // Auto-Cards' state machine sees a clean post-generation
    ...
    // AURA: Flush any queued Frontier requests to `frontier:out` and refresh
    // the human-readable status card. Wrapped in try/catch so a Frontier
    // hiccup can never break Auto-Cards' main flow.
    try { AuraFrontier.flushOut(); } catch (e) { /* swallow */ }
    try { AuraFrontier.status(); }   catch (e) { /* swallow */ }
