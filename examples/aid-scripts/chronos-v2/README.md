...
Chronos is a Ultrascripts showcase script for AI Dungeon: a complete time and
...
season-aware weather simulation. Ultrascripts modules add power when available,
...
- Ultrascripts request polling, acknowledgements, stale request cleanup, and
  graceful fallback to local simulation.

## Setup

1. Load BetterDungeon and open AI Dungeon.
2. Open BetterDungeon -> Ultrascripts and enable Ultrascripts.
...
`Chronos Trace` story cards. It also writes Ultrascripts protocol cards such as
`ultrascripts:out`, `ultrascripts:in:*`, and `ultrascripts:state:scripture`.
