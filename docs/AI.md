...
Simple uses Gemini 3.5 Flash-Lite then 3.1 Flash-Lite for Navigator, Ultrascripts, and Character Prefill. Character Prefill requires structured JSON, so its default route uses the same model family as the Simple connection check. There is no capacity selector in the popup. The internal `quotaStrategy: shared` compatibility setting permits Gemma 4 fallback; saving through the popup keeps the Flash-only default. Navigator requests above an estimated 12,000 tokens including reserved output never fall back to Gemma. Token estimates include prompts, tools, and continuation state and are approximate; a provider context rejection remains terminal.
