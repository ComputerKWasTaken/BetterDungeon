...
Try's success-chance controls and Command's style controls share a right-docked floating pill above `#game-text-input-controller`, **not** its overflow-clipped textarea row: 32px round buttons, compact uppercase labels, focus outlines and live value announcements. The action dock reserves 44px above the controller while a compact pill exists (84px when the history chip is raised above it). The input-history chip reuses the same pill surface on the same right edge, floating a step higher while a mode pill is open so the two never overlap. Try clamps to 5–95% in five-point steps and disables controls at the limits. Command cycles Standard, Subtle and OOC.
