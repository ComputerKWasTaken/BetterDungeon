# Ultrascripts SDK Module - AI Dungeon Test Suite

End-to-end test scripts that exercise the BetterDungeon Ultrascripts `sdk` module
...
| --- | --- | --- | --- |
| `version` | `sdk` | `version` | ok + SDK / BetterDungeon / Ultrascripts version data |
| `config` | `sdk` | `config` | ok + curated BetterDungeon configuration snapshot |

It also verifies:

- the `ultrascripts:heartbeat` card exists
- the heartbeat advertises `sdk` with both `version` and `config`
- `ultrascripts:in:sdk` receives terminal responses
- ack cleanup runs after responses are seen

## Setup

1. Load the BetterDungeon extension and open AI Dungeon.
2. Open BetterDungeon -> **Ultrascripts** and enable Ultrascripts and the **SDK**
   module.
