...
const { ANDROID_ROOT, REPO_ROOT, composeAndroidRuntime, readConfig } = require('../../harness/android-runtime');
...
function testWiringAndMirror() {
  ...
  const mobile = path.join(ASSETS, relative);
  const hash = file => crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex');
  assert.equal(hash(desktop), hash(mobile), 'desktop and Mobile reader files must match');
  const config = readConfig();
  assert.ok(config.scripts.includes(relative));
  assert.ok(fs.existsSync(path.join(ANDROID_ROOT, 'app', 'src', 'main', 'java', 'com', 'computerk', 'betterdungeon', 'InjectionEngine.kt')));
}
