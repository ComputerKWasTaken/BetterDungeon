...
function initAIEndpointSettings() {
  ...
  const message = async request => {
    const response = await window.BetterDungeonPlatform.runtime.sendMessage({ type: 'BETTERDUNGEON_AI', request });
    if (response?.ok) return response.data;
    throw response?.error || new Error('The BetterDungeon AI service did not respond. Reload the extension and try again.');
  };
  ...
  const setTestStatus = (tier, text, state = '') => {
    const el = byId(tier === 'simple' ? 'ai-simple-test-feedback' : 'ai-advanced-test-feedback');
    el.textContent = text;
    el.dataset.state = state;
    el.hidden = !text;
  };
  const testFailure = (tier, error) => {
    const provider = tier === 'simple' ? 'Gemini' : 'the provider';
    switch (error?.code) {
      case 'auth_failed': return `${provider} rejected the API key. Check that the saved key is active.`;
      case 'rate_limit':
      case 'provider_limit': return `${provider} is at a usage limit. Try again later or check your account limits.`;
      case 'not_configured': return `Finish configuring ${provider} before testing.`;
      case 'model_unavailable': return `${provider} could not use the selected model. ${error.message || ''}`.trim();
      case 'timeout': return `${provider} did not respond within 30 seconds.`;
      default: return error?.message || 'The provider did not respond.';
    }
  };
  ...
  async function test(tier) {
    const button = byId(tier === 'simple' ? 'ai-simple-test' : 'ai-endpoint-test');
    const label = button.innerHTML;
    let saved = false;
    try {
      setTestStatus(tier, 'Saving settings…');
      button.textContent = 'Saving settings…';
      await save();
      saved = true;
      button.textContent = 'Testing connection…';
      setTestStatus(tier, 'Testing connection…');
      if (tier === 'advanced' && service === 'custom') {
        let url;
      ...
      const result = await Promise.race([
        message({ op: 'test', tier }),
        new Promise((_, reject) => {
          timeoutId = setTimeout(() => reject({ code: 'timeout', message: 'Connection test timed out.' }), 35000);
        })
      ]).finally(() => clearTimeout(timeoutId));
      setTestStatus(tier, `Connected to ${result.model || (tier === 'simple' ? 'Gemini' : 'the provider')}.`, 'success');
    } catch (error) {
      if (!saved) {
        setTestStatus(tier, error.message || 'Could not save settings.', 'error');
        throw error;
      }
      setTestStatus(tier, `Settings saved. ${testFailure(tier, error)}`, 'error');
    } finally {
  ...
  panel.addEventListener('change', () => { if (!busy) { dirty = true; setStatus('Unsaved changes.'); } });
