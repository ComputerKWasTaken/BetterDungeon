...
(function () {
  ...
  function normalizeConfig(value) {
    ...
    const legacyCaps = [gemini.maxInputTokens, openrouter.maxInputTokens, custom.maxInputTokens]
      .map(normalizeCap);
    const inputCapTokens = normalizeCap(raw.inputCapTokens) ||
  ...
  function resolveLimits(models, settings) {
    ...
    const source = resolved.every(item => item.source === 'discovered') ? 'discovered'
  ...
  function refreshCapabilities(settings) {
    ...
    return hydrateCapabilities().then(() => {
      ...
      if (settings.service === 'gemini') {
        url = `${GEMINI_BASE_URL.replace(/\/openai$/, '')}/models`;
