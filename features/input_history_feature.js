...
class InputHistoryFeature {
  ...
  async init() {
    ...
    this.detectCurrentAdventure(false);
  ...
  async migrateLegacyStorage() {
    ...
    if (!key) return;
    try {
      const result = await chrome.storage.local.get([key, this.legacyStorageKey]);
      const legacy = result?.[this.legacyStorageKey];
      if (result?.[key] === undefined && Array.isArray(legacy)) {
        await chrome.storage.local.set({ [key]: legacy.slice(0, this.maxHistorySize) });
      }
      if (legacy !== undefined) {
        await chrome.storage.local.remove(this.legacyStorageKey);
        this.log('Migrated legacy input history into the current adventure');
      }
    } catch (e) {
  ...
  detectCurrentAdventure(loadHistory = true) {
  ...
  async handleKeydown(e) {
    ...
    // Android Enter/IME keys can insert a newline. Desktop keeps the explicit
    // keyboard-send behavior while touch platforms record only submit clicks.
    if (
      e.key === 'Enter'
      && !e.shiftKey
      && !window.BetterDungeonPlatform?.has('touchControls')
    ) {
