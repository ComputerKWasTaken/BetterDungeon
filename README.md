...
- **Widget** — A Ultrascripts module for dynamic AI Dungeon script widgets. Enables scripts to create HP bars, stats, and game state displays through story-card state.
- **Ultrascripts AI** — Asynchronous `status` and `query` contract for script-driven AI requests. Backend connection is pending.

### ⚡ Automations

- **Custom Dynamic** — Build a personal model pool and route each generation through weighted random, round-robin, or avoid-last model selection. Deprecated models like Wizard are enabled when needed. Inspired by [Zoocata's PRISM](https://play.aidungeon.com/profile/Zoocata_).
